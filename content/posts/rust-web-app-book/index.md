...
※なお、筆者のRustスキルは趣味レベルなのでRustを深く理解したうえでの感想ではありません。間違っている点や他の重要な観点などがある可能性があります。また、個人的なメモ用途で書いているので説明を端折っている箇所がある可能性があります。
...
## まとめ
本書を読むまではRustがWebサーバーサイド開発の銀の弾丸、とまでは言わないがかなりいい選択肢の1つのように思えていた。

しかし実際に本書でRustでサーバーを書いてみるとGoのシンプルさ、手軽さの良さを改めて認識した。

少なくとも今の自分のRustスキルだと（Rustの方が型の表現力が高いとはいえ）Goを採用したほうが長期的にも開発効率が高そうだと感じた。

Rustの基礎文法を習得した自分がこう感じたので、Rustの経験が少ない組織にRustを導入することは少なくとも自分には難しそうだと感じた。[^3]

[^1]: 5章途中までしかやっていないので「はず」と書いている。
[^2]: あるいは単にコンピュータの仕組みへの理解が浅いとも言えるのかもしれない。

[^3]: 言い換えると、要件にもよるがシンプルなWebアプリケーションであれば基本的にはGoの方がビジネス成果を継続的に出しやすそうだと感じた。例えばパフォーマンスが極めて重要だったり、サーバーコストを少しでも圧縮する必要があったり、ドメインロジックが超複雑だったりする場合は選択肢になるかもしれないなと思っている。あとはRustに精通したメンバーの割合がかなり多く、Goよりも組織的に（かつ継続的に）生産性が高まりそうな環境であれば採用の後押しになるかも知れない。
