...
読んで字のごとくですが、配列の長さを型で表現できることが特徴です。

では、配列の長さを型で表現できることでどんなメリットがあるのでしょうか。

### メリット①：仕様を型で表現しやすくなる
たとえば、長さが 4 の `String` の配列は StaticArray を使うと以下のように定義できます。
...
ここで **参加者の名前** および **その試合の最終的なスコア** を `Array String` として扱おうとすると以下のようになるわけですが、これではあらゆる長さの配列をとることができてしまいます。
